...
const PhaseContent: React.FC<Omit<PhaseComponentProps, 'isInitiallyOpen'>> = ({
  phase,
  status,
  prompt,
  setPrompt,
  ...
  <div className="pt-4 mt-4 border-t border-slate-200 dark:border-slate-700">
    ...
    <details className="mt-4 p-4 bg-slate-50 dark:bg-slate-700 rounded-md border border-slate-200 dark:border-slate-600 shadow-inner" open={true}>
      <summary className="text-md font-medium text-slate-700 dark:text-slate-300 cursor-pointer flex justify-between items-center">
...
const PhaseComponent = (props: PhaseComponentProps) => {
  const { phase, title, status, duration, isInitiallyOpen } = props;

  return (
    <details
      data-testid={`phase-component-${phase}`}
      className={`p-4 rounded-lg shadow-md border-2 transition-all duration-300 ease-in-out ${getBorderColor(status)}`}
      open={isInitiallyOpen}
    >
      <summary className="flex justify-between items-center cursor-pointer list-none">
        <h3 className="text-lg font-semibold flex items-center text-slate-700 dark:text-slate-200">
          <span className={`inline-flex items-center justify-center w-6 h-6 rounded-full mr-3 font-bold text-sm
