...
# Test script to call the Kimi API through the local proxy
# Proxy endpoint: http://localhost:3050/v1/chat/completions
#
# Usage: ./test_kimi_proxy.sh <payload_file.json>
# Example: ./test_kimi_proxy.sh payload2.json

# Check if payload file argument is provided
if [ $# -lt 1 ]; then
    echo "Error: Payload file not specified." >&2
    echo "Usage: $0 <payload_file.json>" >&2
    echo "Example: $0 payload2.json" >&2
    exit 1
fi
